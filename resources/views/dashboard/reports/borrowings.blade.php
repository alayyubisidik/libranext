...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900">Borrowings Report</h1>
    <p class="text-sm text-gray-500 mt-1">View and export borrowing data.</p>
</div>
    ...
    <div class="p-6 border-b border-gray-200">
        <form action="{{ route('dashboard.reports.borrowings') }}" method="GET" class="flex flex-wrap gap-4 items-end">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('dashboard.reports.borrowings') }}" class="px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition-colors">Clear</a>
            </div>
        </form>
    </div>

    <div class="p-4 border-b border-gray-200 bg-gray-50 flex gap-2">
        <form action="{{ route('dashboard.reports.borrowings.export') }}" method="GET" class="flex gap-2">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($borrowings as $borrowing)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $borrowing->returned_at ? $borrowing->returned_at->format('Y-m-d') : '-' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-10 text-center text-sm text-gray-400">No borrowings found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
