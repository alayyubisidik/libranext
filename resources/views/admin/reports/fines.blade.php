                    ...
                    <div class="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <form action="{{ route('dashboard.reports.fines') }}" method="GET" class="flex flex-wrap gap-4 items-end">
                            ...
                            <div class="flex gap-2">
                                ...
                                <a href="{{ route('dashboard.reports.fines') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
                                    Clear
                                </a>
                            </div>
                        </form>
                        ...
                        <div class="mt-4 flex gap-2 pt-4 border-t border-gray-200">
                            <form action="{{ route('dashboard.reports.fines.export') }}" method="GET">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Late Days</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($fines as $fine)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $fine->borrowing->borrow_code }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $fine->borrowing->user->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $fine->borrowing->book->title }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $fine->late_days }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rp {{ number_format($fine->amount, 0, ',', '.') }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $fine->created_at->format('Y-m-d') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">No fines found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
